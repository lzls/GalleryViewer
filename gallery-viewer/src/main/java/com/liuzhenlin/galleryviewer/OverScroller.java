...
public class OverScroller {
     ...
     * further and longer, when used with {@link #setFinalX(float)} or {@link #setFinalY(float)}.
     *
     ...
     */
    public void startScroll(float startX, float startY, float dx, float dy, int duration) {
        mMode = SCROLL_MODE;
    ...
    public void fling(
            float startX, float startY, float velocityX, float velocityY,
            float minX, float maxX, float minY, float maxY, float overX, float overY) {
     ...
     */
    public void notifyHorizontalEdgeReached(float startX, float finalX, float overX) {
     ...
     */
    public void notifyVerticalEdgeReached(float startY, float finalY, float overY) {
     ...
     * {@link #fling(float, float, float, float, float, float, float, float, float, float)} method.
     *
     * One should check this value before calling
     * {@link #startScroll(float, float, float, float)} as the interpolation currently in progress
    ...
    public boolean isScrollingInDirection(float xvel, float yvel) {
        final float dx = mScrollerX.mFinal - mScrollerX.mStart;
        final float dy = mScrollerY.mFinal - mScrollerY.mStart;
        return !isFinished() && Math.signum(xvel) == Math.signum(dx)
                && Math.signum(yvel) == Math.signum(dy);
    }

    private static final class SplineOverScroller {
        // Initial position
        float mStart;
        ...
        static final float GRAVITY = 2000.0f;
        ...
        void updateScroll(float q) {
            mCurrentPosition = mStart + q * (mFinal - mStart);
        }
        ...
        private void fitOnBounceCurve(float start, float end, float velocity) {
            ...
            mVelocity = -mDeceleration * totalDuration;
        }
        ...
        private void onEdgeReached() {
            ...
            // The float cast below is necessary to avoid integer overflow.
            final float velocitySquared = mVelocity * mVelocity;
        ...
        boolean update() {
            ...
            mCurrentPosition = mStart + distance;
